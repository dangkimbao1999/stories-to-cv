...
import { demoNow, getDemoSession, getDemoSessionMessages } from "../session-demo-data";
...
export function ChatSessionClient({ sessionId }: { sessionId: string }) {
  ...
  function saveToKnowledgeBase() {
    const selectedSession = session.selectedForKnowledgeBase ? session : markSessionKnowledgeBaseSelection(session, true);
  ...
  return (
    <div className="chat-only-screen">
      ...
      <section className="chat-composer" aria-label="Chat composer">
        ...
        {sources.length > 0 ? <p className="composer-status">{sources.length} source attached to this session.</p> : null}
        {saveNotice ? <p className="composer-status">{saveNotice}</p> : null}

        <textarea
          aria-label="Session message"
          ref={draftMessageRef}
          placeholder="Message Stories to CV"
        />
